    ...
    const checkUsernameUnique = async () => {
      if(username) {
        ...
        try {
          ...
          console.error(axiosError, 'error while checking username in sign-up page');
          if (axiosError?.response?.data?.errors) {
            const errorMessage = Array.isArray(axiosError?.response?.data?.errors) ? axiosError?.response?.data?.errors[0] : axiosError?.response?.data?.errors
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      ...
      <div className="w-full max-w-md space-y-8 bg-white rounded-lg shadow-md p-8">
        <div className="text-center">
          ...
          <p className="mb-4">
            SignUp to start your anonymus advneture
          </p>
        </div>
        <Form {...form}>
        ...
        <div>
          <div className="text-center mt-4">
            <p>
              Already a member?{' '}
              <Link href="/sign-in" className="text-blue-500 hover:underline hover:text-blue-800">
                Sign-in
              </Link>
            </p>
          </div>
